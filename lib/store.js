...
const { forkingToken, combineTokens } = require("./forking-token");

const delayMin = 100;
const delayMax = 10000;

class StateStore {
  constructor(store, current, opt) {
    this.store = store;
    this.current = current;
    this.opt = Object.assign({ maxPending: 20 }, opt || {});
    this._sequence = {};
    this._section = {};
    this._resolving = {};
    // Instance and class (see below)
    this.forkingToken = forkingToken;
    this.combineTokens = combineTokens;
  }

  static async create(store, fallback, opt) {
    return new this(store, await store.load(fallback), opt);
  }
...
// Instance and class (see above)
StateStore.combineTokens = combineTokens;
StateStore.forkingToken = forkingToken;
