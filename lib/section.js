...
class StateSection {
  ...
  makeToken(earlyMut, earlyFixup) {
    ...
    tokens.push(token);
    const maxPending = this.ss.opt.maxPending;

    if (maxPending && tokens.length > maxPending)
      throw new Error(`Too many pending tokens for ${key} (${maxPending})`);

    // Return an async function which must be called in the
    // future to mark this token complete.
    return this.ss.forkingToken(async (lateMut, lateFixup) => {
      token.complete = true;
